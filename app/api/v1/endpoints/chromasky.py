...
from app.services.data_fetcher import data_fetcher  # 导入我们的数据获取器单例
...
def check_data_for_point(
    ...
    """
    # 调用服务来获取数据
    variables = data_fetcher.get_all_variables_for_point(lat=lat, lon=lon)
    
    if "error" in variables:
        raise HTTPException(status_code=500, detail=variables["error"])
        
    time_info = data_fetcher.time_metadata
    
    return {
        "message": "成功获取到以下原始数据",
        "time_info": time_info, # 添加时间信息
